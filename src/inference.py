...
# Streamlit is used for caching in interactive mode, but it may not be
# installed in headless environments (e.g. during automated tests).
# Fallback to a dummy object that provides the methods we use.
try:
    import streamlit as st  # type: ignore
except ModuleNotFoundError:
    import types
    # Provide no‑op implementations for cache and error display
    st = types.SimpleNamespace(
        cache_resource=lambda func=None, **dec_kwargs: (lambda *args, **kwargs: func(*args, **kwargs)) if func else (lambda f: f),
